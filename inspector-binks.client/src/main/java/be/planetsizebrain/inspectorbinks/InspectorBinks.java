...
public class InspectorBinks {
	...
	static {
		...
		try {
			InputStream input = GrowlUtils.class.getResourceAsStream("/libquaqua64.jnilib");
			File temp = File.createTempFile("libquaqua64", "jnilib");
			temp.deleteOnExit();
			OutputStream output = new FileOutputStream(temp);
			IOUtils.copy(input, output);
		    System.load(temp.toString());
		    QuaquaManager.setProperty("Quaqua.jniIsPreloaded", "true");
