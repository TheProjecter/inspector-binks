...
public class Clazz
{
    ...
    public Clazz(File file, boolean inJar) throws ClazzException, IOException
    {
    	this.inJar = inJar;
    	this.file = file;
    	...
    	if (inJar) {
    		init(new ClazzInputStream(new TFileInputStream(file)));
    	} else {
    		init(new ClazzInputStream(new FileInputStream(file)));
    	}
//    	ClazzInputStream cis = new ClazzInputStream(new FileInputStream(file));
//        init(cis);
    }
    ...
    public Clazz getInnerClazz(String inname) throws ClazzException, IOException {
    	if (inJar) {
    		File inner = new TFile(file.getParentFile(), inname + ".class");
    		return new Clazz(file, inJar);
    	} else {
    		String enclosingClassFileName = getFileName();
    		String path = enclosingClassFileName.substring(0, enclosingClassFileName.lastIndexOf(System.getProperty("file.separator")) + 1);
    		return new Clazz(path + inname + ".class");
    	}
    }

    private void init(ClazzInputStream cis) throws ClazzException, IOException
    {
